...
#' @param probs A numeric probability value between 0 and 1 or a vector of probabilities (by default, 0.99)
...
calculate_quantile <- function(gs,
  ...
  if (!is.null(channels)){
    ...
    quantiles <- lapply(channels, function(channel) quantile(expr_mat[,channel], probs=probs, names=FALSE, ...))
    names(quantiles) <- channels
    # Else for all channels
  }else{
    quantiles <- lapply(colnames(gs), function(channel) quantile(expr_mat[,channel], probs=probs, names=FALSE, ...))
